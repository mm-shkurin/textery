...
**Публичный деплой: `<ссылка не заполнена>`.** Адреса прод-копии нет ни в одном
файле репозитория (проверено 15.08.2026) — вписать сюда, как только он есть.
Правила сезона (`.memory-bank/sprint.txt`) обнуляют спринт за нерабочую или
отсутствующую публичную ссылку, так что это место — не украшение. Развернуть свою
копию: см. «Запуск» ниже.
