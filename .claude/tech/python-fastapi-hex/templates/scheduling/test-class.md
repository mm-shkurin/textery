...
- **Queue adapter tests** (`ArqJobQueue`): use `arq`'s `ArqRedis` against a real local Redis test instance (from `infrastructure/.env`), or `arq.worker.Worker` in test mode with `burst=True` to process the queue once and exit.
...
- A test Redis DB index (isolated from dev/prod data) via `infrastructure/.env`
