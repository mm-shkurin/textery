# Usecase Test Template -- Python/FastAPI/Hexagonal
...
- Usecase methods under test are `async def` — test methods calling them must be `async def test_...` (picked up automatically by `pytest-asyncio` in `auto` mode, see `tdd.md`)
