...
- **Known driver/env traps:** a gotchas notes file was referenced here (`infrastructure/notes/acceptance-test-gotchas.md`) but was never actually written for this project — neither `infrastructure/` nor `infra/` has a `notes/` directory. If browser sessions behave inconsistently across tests, diagnose fresh; consider writing the notes file once a root cause is found.
