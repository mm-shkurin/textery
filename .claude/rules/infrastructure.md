...
All infrastructure — local dev, prod-copy, production, certs, DNS, nginx config, observability, CI runners — is managed through files under `infra/` (formerly documented as `infrastructure/` — that directory was always empty; corrected 2026-07-13, see `_project_audit/00_SUMMARY.md`). Read-only investigation is allowed; **state changes are not**.
...
- **All infra changes flow through `infra/`** — container orchestration under `infra/docker-compose.yml`, Dockerfiles under `infra/docker/`. No Terraform/nginx-template/scripts subdirectories exist yet — if that kind of change is needed and no IaC source exists, propose adding one under `infra/` rather than editing anything by hand (see below).
