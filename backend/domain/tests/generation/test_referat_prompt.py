...
from prompt_fixtures import TOPIC, prompt_for
...
def _referat_prompt() -> str:
    ...
    Lowercasing is also why this helper did not move to `prompt_fixtures` with
    `prompt_for`: the ban guards in `test_referat_ban.py` pin the ban sentence
    verbatim, capital `Н` included, so they must not share a folded build.
    """
    return prompt_for(REFERAT).lower()
