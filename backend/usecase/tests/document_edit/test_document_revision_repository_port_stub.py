...
# failure shape -- it must fail loudly if document_edit.document_revision_repository
# stops importing, not skip and let the suite report green.
...
EXPECTED_PORT_METHODS = {
    "find_scope_by_number_and_document": (
        "DocumentRevisionRepository.find_scope_by_number_and_document"
    ),
}

# Discovered across the whole MRO, not from `vars(DocumentRevisionRepository)` alone:
# `vars` reads only the class's own `__dict__`, so a method arriving on a base Protocol
# -- the natural shape once this port is split -- would be invisible here and the roster
# assertion would pass while leaving it uncovered. `typing`/`builtins` bases contribute
# Protocol machinery, not port methods, so they are excluded.
DISCOVERED_PORT_METHODS = sorted(
    {
        name
        for klass in DocumentRevisionRepository.__mro__
...
class AdapterThatForgotToImplementThePort(DocumentRevisionRepository):
    ...
    against `DocumentRevisionRepository` that never implemented a method. It is a real
    subclass, not a Protocol, so it is instantiable and its inherited bodies actually
    run.
    """


class TestDocumentRevisionRepositoryPortStub:
    """Coverage: the DocumentRevisionRepository port bodies raise instead of None.
    ...
    `None` is this port's "no such revision under that document" answer -- so an
    unimplemented adapter would refuse every owner their own revision, silently and
    with the canonical 404 the guard is built to emit. The raising body is the
    enforcement mechanism; every production caller goes through a real implementation
    or a fake, so nothing else ever executes it.
    ...
    def test_should_expose_exactly_the_known_port_methods(self):
        assert sorted(EXPECTED_PORT_METHODS) == DISCOVERED_PORT_METHODS, (
            "DocumentRevisionRepository exposes a method roster this test does not "
            f"cover: {DISCOVERED_PORT_METHODS}"
        )
    ...
    async def test_should_raise_not_implemented_error(self, method_name):
        ...
        assert inspect.iscoroutinefunction(method), (
            f"DocumentRevisionRepository.{method_name} is not an async def, so "
            "awaiting it is not what this test exercised"
        )

        # Called by keyword because the port's scoping arguments are keyword-only.
        # That names this method's own parameters inside a parametrized body, which
        # is deliberate: a second port method with a different signature must fail
        ...
        with pytest.raises(NotImplementedError) as excinfo:
            await method(revision_number=1, document_id=uuid4())
