...
from statements.document_guard_contract import captured_outage
...
class ArrangementSnapshotGuardStatements(RevisionGuardBase):
    ...
    async def act_on_a_store_outside_the_arrangement(self) -> None:
        ...
        await captured_outage(
            self.resolve_via(failing, self.first_document_id),
            f"the outage '{FOREIGN_STORE_OUTAGE_MESSAGE}' to propagate out of the foreign store",
        )
