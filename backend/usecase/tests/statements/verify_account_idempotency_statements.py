...
class VerifyAccountIdempotencyStatements(VerifyAccountStatementsBase):
    ...
    async def given_account_already_verified_once_with_its_code(self) -> None:
        ...
        self.first_consumed_at = self._verification_code_repository.saved_codes[-1].consumed_at
    ...
    async def resubmit_the_same_already_consumed_code(self) -> None:
        ...
        self._clock.fixed_now = self.REPLAY_CLOCK_NOW
    ...
    async def resubmit_the_same_code_after_it_expired(self) -> None:
        ...
        expires_at = self._verification_code_repository.saved_codes[-1].expires_at
        self._clock.fixed_now = expires_at + timedelta(minutes=1)
        await self._execute_verify(self.account_email, self.account_code)
    ...
    def assert_replay_was_a_no_op(self) -> None:
        ...
        assert (
            self._verification_code_repository.saved_codes[-1].consumed_at == self.first_consumed_at
        ), (
            f"expected the stored code's consumed_at to stay the FIRST consume time "
            f"{self.first_consumed_at} (no re-consume), got "
            f"{self._verification_code_repository.saved_codes[-1].consumed_at}"
        )
