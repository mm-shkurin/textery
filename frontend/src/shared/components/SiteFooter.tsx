...
export function SiteFooter({ variant = 'slab' }: SiteFooterProps) {
  return (
    <footer className={`site-footer site-footer-${variant}`} data-testid="site-footer">
      <div className="site-footer-inner">
        <div className="site-footer-brand">
          <img className="site-footer-logo" src="/design/logo-textery.svg" alt="Textery" />
        ...
        {variant === 'slab' && (
          <nav className="site-footer-columns" aria-label="Разделы сайта">
            {COLUMNS.map((column) => (
              <div className="site-footer-column" key={column.title}>
                <h2 className="site-footer-column-title">{column.title}</h2>
                <ul>
