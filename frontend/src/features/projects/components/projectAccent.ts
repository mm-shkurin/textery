...
// Единственный `.ts` в `components/` любого слайса, и это осознанное исключение из раскладки,
// объявленной в README («`components/` — только компоненты, `utils/` — всё остальное»). Файл
// читает `ProjectsPage.module.css`, а CSS-модуль привязан к своему компоненту: из `utils/`
// пришлось бы тянуться в `components/` за стилями, то есть менять нарушение раскладки на
// нарушение направления зависимостей. Таблица живёт рядом с таблицей цветов, которую переводит.
