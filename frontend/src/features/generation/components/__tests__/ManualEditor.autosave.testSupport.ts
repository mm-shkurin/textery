...
// The timer / typing / deferred-promise infrastructure shared by every autosave suite. The two mounted
// starting points (renderCreatedDocument, renderFailedInitDocument) and the constants describing the
// document they create live in ManualEditor.autosaveRender.testSupport.tsx — they are the only helpers
// here that needed JSX, and moving them out is what lets this file be a plain .ts.
