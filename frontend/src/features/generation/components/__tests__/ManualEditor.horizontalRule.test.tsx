import { describe, expect, it, vi } from 'vitest'
import { fireEvent, screen } from '@testing-library/react'
import { paragraphTextNode, renderEditorWithDocumentCreated } from './ManualEditor.testSupport'

vi.mock('../../api/documentApi')

describe('ManualEditor horizontal rule toolbar', () => {
  it('clicking the horizontal-rule toolbar button inserts a divider at the cursor position', async () => {
    await renderEditorWithDocumentCreated()

    const contentArea = screen.getByTestId('editor-content-area')
    contentArea.textContent = 'hello world'
    fireEvent.input(contentArea)

    const textNode = paragraphTextNode(contentArea)
    const cursorRange = document.createRange()
    cursorRange.setStart(textNode, 5)
    cursorRange.setEnd(textNode, 5)
    const selection = window.getSelection()
    selection?.removeAllRanges()
    selection?.addRange(cursorRange)
    fireEvent.select(contentArea)

    const horizontalRuleButton = screen.getByTestId('toolbar-horizontal-rule')
    fireEvent.click(horizontalRuleButton)

    // Block schema: setHorizontalRule splits the paragraph at the cursor and
    // inserts an <hr> block between the two halves.
    expect(contentArea.innerHTML).toBe('<p>hello</p><hr><p> world</p>')
  })
})
