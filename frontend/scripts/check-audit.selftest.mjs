...
// generated from the ledger and cannot drift out of step with it.
...
function expect({ what, report, today = BEFORE, code, quotes = [] }) {
  const dir = mkdtempSync(join(tmpdir(), 'check-audit-'))
  const path = join(dir, 'report.json')
  writeFileSync(path, report)
  checkVerdict({
    what,
    result: runNodeScript(CHECK, [`--report=${path}`, `--today=${today}`]),
    code,
    quotes,
  })
...
expect({
  what: 'a report with no vulnerabilities at all passes',
  report: reportOf({}),
  code: ACCEPTED.length === 0 ? 0 : 1,
  // With a non-empty ledger this is the STALE case: every accepted row describes something the
  // report does not contain. Asserting it here keeps the two halves of the ledger contract in one
  // place — an entry earns its keep only while npm still reports the finding it explains.
  quotes: ACCEPTED.length === 0 ? ['no production advisories'] : ['no longer reports it'],
})
...
reportAndExit({
  ...
  tail: 'clean report, ledger match, expiry, unlisted low, transitive via, unparseable report',
})
