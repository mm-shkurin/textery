...
class ExportControlStatements(ManualEditorStatements):
    ...
    def assert_pdf_and_docx_choices_are_shown(self, driver) -> None:
        self._assert_choice_shown(driver, EXPORT_OPTION_PDF, EXPECTED_PDF_LABEL, "PDF")
        self._assert_choice_shown(driver, EXPORT_OPTION_DOCX, EXPECTED_DOCX_LABEL, "DOCX")

    def _assert_choice_shown(self, driver, locator, expected_label: str, name: str) -> None:
        ...
        assert actual == expected_label, (
            f"expected the {name} export choice to show exactly '{expected_label}', got '{actual}'"
        )
