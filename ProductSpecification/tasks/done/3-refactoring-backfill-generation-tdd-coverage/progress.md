...
- [x] green-acceptance — removed the RED skip marker (and its now-dead `import pytest`); no other change to the test, no production change. Full backend acceptance suite GREEN against the wired backend (GENERATION_PROVIDER=fake, local Postgres 16): 5 passed, 0 skipped — including `test_should_create_pending_then_return_completed_document_content`.
