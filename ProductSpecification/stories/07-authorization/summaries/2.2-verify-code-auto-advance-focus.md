...
**Quirk:** Multiple frontend-serving processes can coexist on this host — a stray `npm run dev` from another worktree squats port 5173, and the docker frontend container (port 80) may be running a stale build that predates current source.
