...
- [ ] red-frontend (coverage: applyRegisterError ignores non-duplicate-email error codes)
- [ ] green-frontend (coverage: applyRegisterError ignores non-duplicate-email error codes)
- [ ] red-frontend-api (coverage: registerApi.register maps a real non-ok HTTP response to RegisterApiError — previously marked [S] on a false premise, see test-coverage note below; generationApi.test.ts covers a separate, unrelated inline postJson in the generation feature, not auth's shared httpClient.ts)
- [ ] green-frontend-api (coverage: registerApi.register maps a real non-ok HTTP response to RegisterApiError)
- [ ] red-frontend-api (coverage: postJson throws HttpError with status+body on non-ok response, falling back to {} when body isn't valid JSON)
- [ ] green-frontend-api (coverage: postJson throws HttpError with status+body on non-ok response, falling back to {} when body isn't valid JSON)
