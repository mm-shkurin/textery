...
- [ ] green-frontend (DESIGN TENSION, do not silently resolve: LoginForm's disabled-button (2.3) and loading-indicator (3.2) behavior currently ride on `useSubmitPlaceholder`'s 500ms fake delay, and `LoginForm.test.tsx` exercises both WITHOUT mocking loginApi — once the placeholder is replaced with the real promise, those tests will invoke the unmocked `login` and hit `Not implemented`. Replace the placeholder's delay with the real `loginApi.login` promise while preserving both tested behaviors (5.1's RegisterForm precedent: local `isSubmitting` + `handleSubmit`), do NOT add a competing submit path; `LoginForm.test.tsx` will likely need to mock loginApi too. Also: LoginForm's inputs are uncontrolled with no refs — green needs refs or state to read entered values.
  REVIEW FINDINGS from red-frontend's concurrent agent-review + premortem passes on commit `ff2e0b4` (both CONCERNS, non-gating, surfaced not fixed — green must resolve or explicitly dismiss each):
  (a) VACUOUS GUARD, highest priority, same class already shipped once in Scenario 4.1 — `LoginForm.invalidCredentials.test.tsx` asserts `queryByTestId('login-email-error')`/`('login-password-error')` are absent, but neither testid exists anywhere in the repo, so both pass unconditionally for every implementation forever. The "not a per-field error" claim is actually carried only by the `queryAllByTestId('login-form-error')).toHaveLength(1)` line. Fix: target locators that exist (reuse RegisterForm's real per-field error testid naming) or drop the two unbacked lines.
  (b) NON-CONTRACT REJECTIONS UNCONSTRAINED — both passes flagged this independently, and it is the sharpest finding. No test reads `errorCode`, so the minimal green satisfying every test is `catch (e) { setError(e.message) }`, which renders ANY rejection verbatim. premortem traced the concrete incident: `postJson` only builds an `HttpError` inside `if (!res.ok)`, so a network failure rejects with a bodyless `TypeError: Failed to fetch`; registerApi's `httpError.body.error_code` pattern then throws a second TypeError from inside the catch, and unconditional pass-through renders that internal string into `login-form-error` on the app's most security-sensitive surface. test-review's earlier rejection of "forcing an errorCode branch" conflated two things: asserting WHICH distinct message 5.3 shows is out of scope, but asserting that a non-contract rejection does not render as user-facing text is 5.2's own non-disclosure clause. The endpoints.md hazard-scan group 7 guarantee holds only for values the backend actually produced — a leaked value never came from the backend, so group 7 never covers it. Guard: reject with `new TypeError('Failed to fetch')` and with a `message`-less contract error; assert `login-form-error` is absent or holds a client-owned generic constant.
  (c) THE 2.3/3.2 TESTS WILL GO FALSELY GREEN, not fail — sharper than the design tension noted above. `LoginForm.test.tsx:69-101` exercises disabled-button/loading-indicator without mocking loginApi; after the placeholder swap both still PASS, because click → isSubmitting true → disabled → promise REJECTS (`Not implemented`) → finally → re-enabled satisfies `waitFor(not.toBeDisabled())`. The guards silently re-bind to the failure path and would keep passing even if green never wired `login` at all. Fix: mock loginApi in `LoginForm.test.tsx` and assert against a manually-controlled pending promise (the `resolveLogin` pattern in the new file), binding to a real in-flight boundary and a resolved success path.
  (d) DOUBLE-SUBMIT REENTRANCY — `useSubmitPlaceholder.ts:16` carries `if (isSubmitting) return`; green rewrites handleSubmit inline (RegisterForm precedent re-implements by hand). If green drops that line every test still passes: no test fires two synchronous clicks against a pending call. Incident: two POSTs, two failed-attempt increments, Scenario 5.4's lockout fires at half the intended threshold. Guard: two back-to-back `fireEvent.click` against an unresolved login mock, assert `toHaveBeenCalledTimes(1)`.
  (e) TEST 6's SYNC POINT is on call, not resolution — `await waitFor(() => expect(api.login).toHaveBeenCalledTimes(1))` fires when `login` is invoked (synchronously, before its await), so test 6 largely re-guards test 5's "no error while pending". Test 7 already uses the correct sync point (`waitFor(() => expect(submitButton).not.toBeDisabled())`); test 6 should match.)
