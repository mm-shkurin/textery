...
- [~] red-usecase (coverage: rollback-of-rollback — unit_of_work.rollback() itself raising still surfaces RegistrationFailedException, not the secondary exception) — closes the open follow-up flagged by both agent-review and premortem on the red-usecase commit
- [ ] green-usecase (coverage: rollback-of-rollback — unit_of_work.rollback() itself raising still surfaces RegistrationFailedException, not the secondary exception)
