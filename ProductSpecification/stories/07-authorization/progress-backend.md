...
- [~] adapters-discovery — must cover: SqlAlchemyUnitOfWork adapter sharing one AsyncSession with both repositories (premortem gap 2, restated on the final coverage commit's premortem — the usecase-layer 100% coverage claim gives false confidence since fakes always share state trivially and can't catch a session-mismatch bug); account_storage.save() rolling back on non-IntegrityError DB failures, not just IntegrityError (premortem gap 1)
- [ ] green-acceptance
