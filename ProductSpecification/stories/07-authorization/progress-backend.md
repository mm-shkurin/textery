...
- [~] adapters-discovery — must cover: SqlAlchemyUnitOfWork adapter sharing one AsyncSession with both repositories (premortem gap 2); account_storage.save() rolling back on non-IntegrityError DB failures, not just IntegrityError (premortem gap 1) — must cover: SqlAlchemyUnitOfWork adapter sharing one AsyncSession with both repositories (premortem gap 2); account_storage.save() rolling back on non-IntegrityError DB failures, not just IntegrityError (premortem gap 1)
- [ ] green-acceptance
