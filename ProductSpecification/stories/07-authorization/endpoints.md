...
| POST | /api/v1/auth/resend-code | _Planned, not yet implemented_ — issue a new verification code (60s cooldown) |
...
- Verification code is returned directly in the `register` response body (and `resend-code`
  once that endpoint is implemented) (mocked — no real email is sent), per `07_Authorization.md`.
