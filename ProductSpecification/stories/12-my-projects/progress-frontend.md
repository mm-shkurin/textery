...
Scenarios from `tests/02_UI_Tests.md` (34 after the 2026-08-06 backend/frontend spec merge —
the nine added scenarios are listed unstarted at the end of this file). The feed reads
`GET /api/v1/projects`, which now exists on `dev` and returns `items` only; the retry control
posts `POST /api/v1/generations/{id}/retry` (**not** `/repeat` — the backend contract won the
merge, see `progress.md` Decisions), which does **not** exist yet. Frontend work builds against
a mock of the missing half; a `red-selenium`/`green-selenium` leg is backend-gated whenever the
behaviour it drives is not yet served, and is then deferred `[S]`
