...
- [S] red-frontend-api — no API call: formatting is client-side editor state only, no backend endpoint involved
- [S] green-frontend-api — same reason
- [~] red-frontend-cursor-fix — confirmed defect (agent-review + premortem on green-frontend commit, verified against `tests/02_UI_Tests.md:150-153`): spec requires cursor-on-line (no selection) to blockquote the whole line; shipped `toggleMark('blockquote')` is selection-only and no-ops on a collapsed cursor. Add failing test: cursor placed mid-line with no selection, click blockquote button, expect whole line wrapped in blockquote.
