...
- [~] green-frontend — one line in shape: call `onSaved()` on the suppression path (the document genuinely IS clean) instead of returning bare, or have the guard report "clean" to the caller so `ManualEditor` settles the flag.
