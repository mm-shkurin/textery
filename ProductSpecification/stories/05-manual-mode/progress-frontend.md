...
- [S] red-frontend-api — no API call: formatting is client-side editor state only, no backend endpoint involved
- [S] green-frontend-api — same reason
- [ ] red-frontend-coverage — `ManualEditorToolbar.tsx:34`'s `disabled={editor ? action.disabled?.(editor) ?? false : true}` null-editor branch (`: true`) is untested — toolbar renders with `editor === null` before the Tiptap instance is ready. **agent-review correction:** this branch disables EVERY `TOOLBAR_ACTIONS` button when `editor === null` (bold, italic, strike, etc.), not just undo/redo — the test must assert ALL rendered toolbar buttons are disabled with a null editor, not just `toolbar-undo`/`toolbar-redo`, or the coverage gap is only partially closed.
- [ ] green-frontend-coverage — no production change expected (branch is already correct); test-only addition should make the branch green.
