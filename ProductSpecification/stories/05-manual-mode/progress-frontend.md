...
- [ ] green-frontend — version guard on autosave path. Add a finite-version parse guard at the **documentApi.ts parse boundary** (shared `parseVersion`/wire-parse throwing `INVALID_VERSION_MESSAGE` when `!Number.isFinite`), applied to `createDocument`, `getDocument`, AND `putDocument`/`saveDocument` responses so the refetch-retry path (line 111) is guarded too. Un-skip the 4 `documentApi.versionFailClosed.test.ts` cases. **RED-FIRST (agent-review + premortem CONCERNS on f862710, converged):** the current RED drives `createDocument` ONLY — the real silent-overwrite path is `getDocument`/`putDocument` and the 409→refetch→retry (documentApi.ts:111-112, where a garbage refetched version is re-PUT). Before implementing, ADD failing RED cases for `getDocument` (null/"abc"/absent version → rejects) and the 409-refetch branch (garbage refetched version → saveDocument rejects, never PUTs it); otherwise get/put guards ship as untested branches. Also: export `INVALID_VERSION_MESSAGE` from production and have the test import it (avoid the two-copy drift the test's inline literal invites). Drop or keep-with-note the `NaN` case (unreachable over real `response.json()`; redundant with `"abc"` under `Number.isFinite`).
