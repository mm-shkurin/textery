...
- [~] red-frontend-selection-restore — confirmed defect (agent-review on cursor-fix commit): after the cursor-only toggle, the selection is left expanded to the whole block (`setTextSelection` widens it, nothing collapses it back). Next keystroke after clicking blockquote with a collapsed cursor overwrites the entire line instead of inserting at the original position — data-loss-adjacent regression, currently unguarded. Add failing test: cursor collapsed mid-word, click blockquote, then simulate typing a character; assert only the cursor position is affected (selection collapsed back to original offset), not the whole line replaced.
- [ ] green-frontend-selection-restore — after `toggleMark`, restore/collapse the selection to the original cursor offset (capture offset before expanding, `setTextSelection({from: originalOffset, to: originalOffset})` after the toggle).
- [ ] align-design
- [S] green-selenium — backend unavailable on this branch (backend developed in parallel session/branch); no live app to drive Selenium against
- [S] demo — same reason, no live backend to drive a visible Selenium run against
