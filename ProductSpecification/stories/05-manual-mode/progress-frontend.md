...
- [S] red-frontend-api — no API call: formatting is client-side editor state only, no backend endpoint involved
- [S] green-frontend-api — same reason
- [ ] red-frontend-parseHTML — new file `ManualEditor.heading3.parseHTML.test.tsx`, mirroring `ManualEditor.codeBlock.parseHTML.test.tsx`'s pattern: mock `getDocument` with `before<h3>hello world</h3>after`, load via `existingDocumentId`, expect exact round-trip. Expected to pass immediately (see align-design note — the round-trip was verified working by direct probe), so this is a characterization test locking in current behavior, not a true red.
- [ ] green-frontend-parseHTML — expected no-op; if the test does fail, `heading3Mark.ts`'s `parseHTML` needs the same treatment 7.4's `<pre>` did (match a child tag conditioned on its parent, since ProseMirror's hardcoded `blockTags` handling can drop a wrapper tag with no matching rule).
