...
- [~] green-frontend — **must decide the reject/error path before implementing** (premortem finding): what happens to a queued "save again" request if the in-flight save's promise rejects? Two bad defaults to avoid: (a) silently dropping the queued edit on error, (b) leaving isSaving/spinner stuck disabled forever. Pick and test both branches (resolve-then-queued-fires, reject-then-queued-behavior) in this step.
- [ ] red-frontend-api
- [ ] green-frontend-api
- [ ] align-design
- [ ] green-selenium
