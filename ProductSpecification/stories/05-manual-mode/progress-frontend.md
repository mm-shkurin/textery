...
- [~] green-frontend — one line in shape: call `onSaved()` on the suppression path (the document genuinely IS clean) instead of returning bare, or have the guard report "clean" to the caller so `ManualEditor` settles the flag. **Review-pass pre-conditions on `4dbe3a4` (both CONCERNS, non-gating) — the RED takes every assertion on the HAPPY path only (one clean autosave, no error, nothing in flight), so the minimal green `if (isAlreadySaved(...)) { onSaved(); return }` satisfies it while behaving badly in two states the test never enters:**
