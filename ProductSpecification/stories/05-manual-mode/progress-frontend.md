...
- [~] green-frontend — debounce/coalesce + **dirty-guard**: suppress the autosave PUT when nothing is unsaved (fixes the redundant trailing PUT above). Verify the base coalesce is covered by E3.1 impl — `[S]` that part if reuse. **Review-pass pre-conditions (both passes over `8e20951`, CONCERNS — green must add these RED-first before/alongside the guard):**
