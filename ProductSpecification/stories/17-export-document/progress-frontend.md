...
- [x] red-frontend-api
- [x] green-frontend-api — implemented via shared transport `responseType: 'blob'` (httpClient `performRequest` returns `res.blob()` AFTER the `res.ok` guard); `exportDocument` now `Promise<Blob>` through `send`/`authorizedRequest`, inheriting res.ok→HttpError, 401 renew+replay, and withTimeout. 508/508 pass. Owed non-ok/401 red cases now lower priority — the guards are the shared transport's own (covered by its tests) — but an export-level assertion remains nice-to-have. **original directive from reviews on 9f6f0d9 (both agent-review + premortem):** implement `exportDocument` by adding a **blob-returning variant to the shared authorized transport** (send/authorizedRequest/httpClient), NOT a bare `fetch().blob()`. A hand-rolled fetch bypasses three guards the stack owns: (a) `res.ok` check — without it a 404/500 error body is streamed and downloaded *as the document* (corrupt success); (b) 401 → single-flight session renew+replay — without it an expired ~15-min access token yields a garbage download with no `SessionExpiredError`; (c) `withTimeout`. The existing `request` path does `res.json()` unconditionally, so a binary path must be added there rather than around it. Change signature to `Promise<Blob>`. **Owed red cases (add at green-frontend-api or its coverage step):** exportDocument REJECTS on non-ok status (404/500), and a 401 renews+replays once (mirror getDocument's 401 test).
- [ ] align-design
