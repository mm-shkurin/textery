...
- [~] green-frontend — render an `export-spinner` element while `isExporting` (removed on settle via existing `.finally`), then remove the `describe.skip`. Must NOT break 2.1's disabled-options lock / `wait_for_export_in_flight`. **green guidance (premortem ed1a7a1, PASS):** render the spinner as an INLINE descendant of the `ExportControl` root — `{isExporting && <span data-testid="export-spinner">…}` — NOT a React portal to `document.body` (a portal satisfies jsdom `toBeVisible` but fails the selenium `MANUAL_EDITOR_SELECTOR`-scoped descendant selector). Gate on `isExporting` (which the existing `.catch().finally(setIsExporting(false))` already clears on BOTH resolve and reject), so the error-path removal comes free before 3.2 lands.
- [ ] red-frontend-api
- [ ] green-frontend-api
- [ ] align-design
- [ ] green-selenium
