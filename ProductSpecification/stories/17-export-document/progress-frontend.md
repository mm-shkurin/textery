...
- [~] green-frontend — render an `export-spinner` element while `isExporting` (removed on settle via existing `.finally`), then remove the `describe.skip`. Must NOT break 2.1's disabled-options lock / `wait_for_export_in_flight`.
- [ ] red-frontend-api
- [ ] green-frontend-api
- [ ] align-design
- [ ] green-selenium
