...
- [~] green-frontend  <!-- must implement: read code from useSearchParams, loading state, oauthExchange({code}) via shared apiError boundary, saveSession, navigate(safeRedirectTarget,{replace:true}) ONLY if saveSession returned true (fail-closed), wire /auth/callback route in App.tsx, un-skip -->
