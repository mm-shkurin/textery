...
## Delivery note — 2026-08-21: the rest of this story shipped as production code

Scenario 1.1 above completed the full TDD cycle. The remaining scenarios in this file did
not, and this is a deliberate, developer-instructed departure rather than a lapse: at the
per-scenario rate this story was running (six work units each, 170 scenarios left) the
sprint deadline was unreachable, so the behaviour was written directly as production code.

Every `[ ]` below is therefore rewritten to `[S]`. `[S]` here means "the BEHAVIOUR this
scenario describes is implemented; the TDD cycle for it was not run" — it does not mean
the scenario was dropped. The scenario text remains the specification of what the code is
supposed to do, and it is the natural backlog for a later coverage pass.

What was implemented, and where it lives:

| Scenarios | Code |
|---|---|
| §1–§6 (ingest, identity, payload, idempotency, rate) | `adapters/rest/src/router/analytics/`, `usecase/src/analytics/record_analytics_event.py`, `domain/src/analytics/analytics_payload.py` |
| §7 (registration attribution + technical context) | `domain/src/analytics/{attribution,device,language_tag,technical_context}.py`, `usecase/src/analytics/registration_context_recorder.py` |
| §8 (registration/sign-in events, incl. OAuth) | `usecase/src/auth/verify_account.py`, `login_user.py`, `auth/oauth/complete_oauth_callback.py` |
| §9 (generation + save events) | `usecase/src/generation/{request_generation,generate_document}.py`, `usecase/src/document/save_document.py` |
| §10–§13 (ordering, erasure, fail-open, disclosure) | the schema (`sequence`, FK `ON DELETE SET NULL`) and `adapters/db/src/access/analytics/server_event_recorder.py` |
| Integration §1–§2 (geolocation, OAuth handshake) | `adapters/geolocation_provider/`, `usecase/src/analytics/oauth_attribution_parking.py` |
| Infra §3 (configuration, named defaults) | `application/src/app/container/analytics_wiring.py`, `backend/.env.example` |

**Two honest gaps, and the second is the one that matters.** No load scenario was run.
And nothing here has been executed against a real Postgres: the Docker daemon was down on
the machine this landed from, so `adapters/db` tests skipped by prerequisite and
`alembic upgrade head` was never run. The three new migrations and the four new storage
adapters are the part of this story that no test has touched — verify those first.
...
- [S] red-adapter rest — `POST /api/v1/analytics/events`, `204 No Content`, no body.
...
- [S] green-adapter rest — also adds `("POST", "/api/v1/analytics/events")` to
...
- [S] red-usecase
- [S] green-usecase
- [S] adapters-discovery — **do not mark db `[S]` on the grounds that the port is already
