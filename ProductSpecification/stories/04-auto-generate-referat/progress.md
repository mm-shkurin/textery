...
## Функциональное состояние на 2026-08-06 (после мерджа в dev)

**Реферат теперь реально доходит до модели.** Шаблон `_referat` со структурой
(введение — актуальность и цель, основная часть, заключение — выводы) и запретом
на выдуманные источники существовал, был закреплён голденом — и **не
использовался**: `GenerateDocument` строил промпт через `build_prompt` и
выбрасывал результат, а `GigaChatProvider.generate` брал сущность и лепил
собственный f-string. Порт провайдера теперь принимает готовый промпт, композер
остался один, и он в домене.

**Тип стал выбираемым.** В `shared/documentTypes.ts` реферат стоял
`available: false` — карточка с плашкой «скоро». Флаг снят.

Написано вне TDD-цикла; чекбоксы сценариев выше не отмечены.
